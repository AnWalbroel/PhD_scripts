...
if sys.argv[1] == 'hatpro':
	o_angle = 4.0		# MWR sensor opening angle (to each side) for HATPRO
...
include_IR = True				# if true, IR camera geometry is added to mwr_mir_sea_sfc plot
...
mir_angle = -10.0						# mirror angle in deg; counter-clockwise is > 0
...
if view_3D:
	...
	if view == 'full':
		...
		axis_lims_x = [np.floor(np.min([SHP1[0], SHP1_mirac[0]]))-3.0, np.ceil(np.max([SHP2[0], SHP2_mirac[0]]))+3.0]
		axis_lims_y = [-0.5*(axis_lims_x[1]-axis_lims_x[0]), 0.5*(axis_lims_x[1]-axis_lims_x[0])]
		axis_lims_z = [-20.0, 2.5]
	...
	a1.plot(np.array([SHP1[0], SHP2[0]]), np.array([SHP1[1], SHP2[1]]), np.array([SHP1[2], SHP2[2]]),
				linewidth=1.5, color=(0.8,0,0), label='MWR')
	a1.scatter(np.array([SHP1[0]]), np.array([SHP1[1]]), np.array([SHP1[2]]), s=50.0, marker='<', color=(0.8,0,0))
	a1.scatter(np.array([SHP2[0]]), np.array([SHP2[1]]), np.array([SHP2[2]]), s=50.0, marker='>', color=(0.8,0,0))
	a1.text(SHPC[0], SHPC[1], SHPC[2], f"{SHP2[0]-SHP1[0]:.1f}" + "$\,$m", color=(0.8,0,0), 
			fontsize=fs_small, ha='center', va='top')
	...
	hatpro_ellipsis = np.array([SHPC[0] + hatpro_ell_x, hatpro_ell_y1, -20.0*np.ones(hatpro_ell_x.shape)])
	hatpro_ellipsis2 = np.array([SHPC[0] + hatpro_ell_x, hatpro_ell_y2, -20.0*np.ones(hatpro_ell_x.shape)])
	...
	a1.plot(np.array([SHP1_mirac[0], SHP2_mirac[0]]), np.array([SHP1_mirac[1], SHP2_mirac[1]]), np.array([SHP1_mirac[2], SHP2_mirac[2]]),
				linewidth=1.5, color=(0.8,0,0), label='MWR')
	a1.scatter(np.array([SHP1_mirac[0]]), np.array([SHP1_mirac[1]]), np.array([SHP1_mirac[2]]), s=50.0, marker='<', color=(0.8,0,0))
	a1.scatter(np.array([SHP2_mirac[0]]), np.array([SHP2_mirac[1]]), np.array([SHP2_mirac[2]]), s=50.0, marker='>', color=(0.8,0,0))
	a1.text(SHPC_mirac[0], SHPC_mirac[1], SHPC_mirac[2], f"{SHP2_mirac[0]-SHP1_mirac[0]:.1f}" + "$\,$m", color=(0.8,0,0), 
			fontsize=fs_small, ha='center', va='top')
	...
	mirac_ellipsis = np.array([SHPC_mirac[0] + mirac_ell_x, mirac_ell_y1, -20.0*np.ones(mirac_ell_x.shape)])
	mirac_ellipsis2 = np.array([SHPC_mirac[0] + mirac_ell_x, mirac_ell_y2, -20.0*np.ones(mirac_ell_x.shape)])
