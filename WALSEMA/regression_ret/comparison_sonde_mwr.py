...
"""
	Visualizing and comparing radiosonde data with MWR (HATPRO in this case).
	IWV, maybe LWP, temperature and humidity profiles will be covered.
	- load HATPRO regression retrieval coefficients, TB and radiosonde data
	- retrieve IWV, ... from TBs
...
# paths:
path_data = {'radiosondes': "/mnt/d/Studium_NIM/work/Data/WALSEMA/radiosondes/",
			'hatpro': "/mnt/e/HATPRO/Y2022/",
...
n_freq_bl = 7
n_angle = 6
mwr_dict = {'tb': np.full((n_sec, n_freq), np.nan),
			'time': np.zeros((n_sec,)),
			'time_npdt': np.full((n_sec), np.datetime64("1970-01-01T00:00:00")),
			'freq': np.array([22.240, 23.040, 23.840, 25.440, 26.240, 27.840, 31.400,
							51.260, 52.280, 53.860, 54.940, 56.660, 57.300, 58.000])}
mwr_bl_dict = {'tb': np.full((n_t_bl, n_freq_bl, n_angle), np.nan),
			'time': np.zeros((n_t_bl,)),
			'time_npdt': np.full((n_t_bl), np.datetime64("1970-01-01T00:00:00")),
			'ele': np.array([90, 14.4, 11.4, 8.4, 6.6, 5.4]),
			'freq': np.array([51.260, 52.280, 53.860, 54.940, 56.660, 57.300, 58.000])}
t_idx = 0
t_idx_bl = 0
while nowdate <= enddate:
	...
	nd_day = nowdate.day
	folder_date = path_data['hatpro'] + f"M{nd_month:02}/D{nd_day:02}/"

	# look for netCDF files... several file conventions are of interest:
	nc_files = sorted(glob.glob(folder_date + f"{str(nowdate.year)[-2:]}{nd_month:02}{nd_day:02}.BRT.NC"))
	if nowdate == dt.datetime(2022,7,30): nc_files = []		# exception to avoid calibration test measurement TBs
	ele_90_files = sorted(glob.glob(folder_date + f"ELE90_{str(nowdate.year)[-2:]}{nd_month:02}{nd_day:02}.BRT.NC"))
	if len(ele_90_files) > 0:
		nc_files.append(ele_90_files[0])

	# also look for BL scan files:
	nc_bl_files = sorted(glob.glob(folder_date + f"{str(nowdate.year)[-2:]}{nd_month:02}{nd_day:02}.BLB.NC"))
	ele_90_bl_files = sorted(glob.glob(folder_date + f"ELE90_{str(nowdate.year)[-2:]}{nd_month:02}{nd_day:02}.BLB.NC"))
	if len(ele_90_bl_files) > 0:
		nc_bl_files.append(ele_90_bl_files[0])

	# loop through files and concat TBs and time manually:
	for file in nc_files:
		TB_DS = xr.open_dataset(file)
		n_time = len(TB_DS.time.values)

		mwr_dict['tb'][t_idx:t_idx+n_time,:] = TB_DS.TBs.values
		mwr_dict['time_npdt'][t_idx:t_idx+n_time] = TB_DS.time.values
	...
	# repeat for BL files:
	for file in nc_bl_files:
		TB_DS = xr.open_dataset(file)
		n_time_bl = len(TB_DS.time.values)

		mwr_bl_dict['tb'][t_idx_bl:t_idx_bl+n_time_bl,:,:] = TB_DS.TBs.values[:,7:,:-1]		# last angle is 0 deg, whyever
		mwr_bl_dict['time_npdt'][t_idx_bl:t_idx_bl+n_time_bl] = TB_DS.time.values
...
mwr_dict['tb'] = mwr_dict['tb'][:t_idx,:]
mwr_dict['time_npdt'] = mwr_dict['time_npdt'][:t_idx]
mwr_dict['time'] = mwr_dict['time'][:t_idx]

mwr_bl_dict['tb'] = mwr_bl_dict['tb'][:t_idx_bl,:,:]
mwr_bl_dict['time_npdt'] = mwr_bl_dict['time_npdt'][:t_idx_bl]
mwr_bl_dict['time'] = mwr_bl_dict['time'][:t_idx_bl]

# also need to reorganize BL tb array: the second axis must be the elevation angles
mwr_bl_dict['tb'] = np.swapaxes(mwr_bl_dict['tb'], 1, 2)
...
ret_vars = ['prw', 'clwvi', 'ta', 'hua', 'ta_bl']
RET_DS = xr.Dataset({	'ta': (['time', 'height'], np.full((t_idx, n_height_ret), np.nan)),
						'hua': (['time', 'height'], np.full((t_idx, n_height_ret), np.nan)),
						'ta_bl': (['time_bl', 'height'], np.full((t_idx_bl, n_height_ret), np.nan)),
					...
					coords=	{	'time': (['time'], mwr_dict['time_npdt']),
								...
								'height': (['height'], COEFF_DS.height.values)})
K_idx = select_MWR_channels(mwr_dict['tb'], mwr_dict['freq'], band='K', return_idx=2)
V_idx = select_MWR_channels(mwr_dict['tb'], mwr_dict['freq'], band='V', return_idx=2)

# build observation matrix according to the regression (for each height, if a profile
# is considered).
for var in ret_vars:
	if var not in ['ta', 'ta_bl']:
		K_reg_obs = build_K_reg(mwr_dict['tb'][:,K_idx], order=2)
	elif var == 'ta':
		K_reg_obs = build_K_reg(mwr_dict['tb'][:,V_idx], order=2)
	else:
		K_reg_obs = build_K_reg(np.reshape(mwr_bl_dict['tb'], (t_idx_bl, n_freq_bl*n_angle)), order=2)

	if var in ['ta', 'hua']:
		for ii in range(n_height_ret):
			RET_DS[var][:,ii] = K_reg_obs.dot(COEFF_DS[f"c_{var}"].values[:,ii])

	elif var == 'ta_bl':
		for ii in range(n_height_ret):
			RET_DS[var][:,ii] = K_reg_obs.dot(COEFF_BL_DS[f"c_{var}"].values[:,ii])
