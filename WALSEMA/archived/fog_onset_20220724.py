...
# import data:
file = path_data['mirac-p'] + "M07/D24/ELE90_220724.BRT.NC"
MIR_DS = xr.open_dataset(file)

file = path_data['hatpro'] + "M07/D24/ELE90_220724.BRT.NC"
...
# hatpro TBs:
K_band_idx = select_MWR_channels(HAT_DS.TBs.values, HAT_DS.frequencies.values, band='K', return_idx=2)
for kk, ff in enumerate(HAT_DS.frequencies.values[K_band_idx]):
	a_hat.plot(HAT_DS.time.values, HAT_DS.TBs.values[:,kk], linewidth=1.2, label=f"{ff:.2f} GHz")
...
# plt.show()
f1.savefig(path_plots + "WALSEMA_hatpro_mirac-p_skycam_fog_onset_20220724.pdf", bbox_inches='tight')