...
then
  #gzip, move file
  echo $(date +"%Y-%m-%d %H:%M:%S") moving " $monthDay.$mrrProductSuffix"
  /usr/bin/mv "$mrrPath/$mrrProduct/$longYearMonth/$monthDay.$mrrProductSuffix" $archivePath/$ourProduct/$year/$month/$day/"$year$month$day"_"$site"_"$ourProduct".txt
  echo $(date +"%Y-%m-%d %H:%M:%S") zipping "$year$month$day"_"$site"_"$ourProduct".txt
