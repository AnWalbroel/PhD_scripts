...
def calc_radar_top_height(radar_mask, data_flag):
    ...
    pc_pixel = (data_flag_r == Radar_flag.good).where(
        ...
        | (radar_mask == Cloud_flag.certain),  # probably  # certain pixe
        drop=True,
    )
    assert np.all(
        pc_pixel.height.diff("height") < 0
    ), "dBZ.height is not strong monotonously decreasing."
...
def length_of_True_chunks(mask):
    ...
    cc = np.concatenate(
        (
            ...
            [True],  # add end index. This is only considered by the [::2] slice if it belongs to a False chunk
        )
    )
...
def find_sampling_gaps_narval(data_flag):
    ...
    gap_mask[
        same_as_previous | same_as_next
    ] = False  # A gap must be only one time step. If if the previous or next were also gaps, remove this gap.
    # Check that there is always no more than one consecutive gap time step
    assert (
        length_of_True_chunks(gap_mask).max() <= 1
    ), "something went wrong in constraining the gaps"
...
def make_HAMP_cloudmask(
    ...
    if "20200122" in radar_name:  # radar was not working
        ...
        assert (
            ...
            == "1: noise; 2: surface; 3: sea; 4: radar calibration; 5: side lobes removed"
        ) or (
        ...
        if radar_name[-23:-10] in ("/radar_201312", "/radar_201608"):  # NARVAL
            assert np.isneginf(
                dBZ
            ).any(), 'neginf should be used to mark "measured, but nothing seen, i.e. signal below noise detection" pixels'

            data_flag.values[np.isneginf(dBZ) & (data_flag == Radar_flag.good)] = Radar_flag.clear
            data_flag.values[
                np.isnan(dBZ) & (data_flag == Radar_flag.good)
            ] = Radar_flag.no_measurement

            gap_mask = find_sampling_gaps_narval(data_flag)
        else:  # EUREC4A
            assert not np.isneginf(
                dBZ
            ).any(), 'neginf should not be used anymore to mark "measured, but nothing seen, i.e. signal below noise detection" pixels'

            radar_raw = xarray.open_dataset(radar_name, mask_and_scale=False)
            assert (
                radar_raw.dBZ.missing_value == -888.0
            ), '-888. should have been used to mark "measurement, but nothing seen"'

            data_flag.values[
                (radar_raw.dBZ == radar_raw.dBZ.missing_value) & (data_flag == Radar_flag.good)
            ] = Radar_flag.clear
            assert np.isnan(
                radar_raw.dBZ._FillValue
            ), "_FillValue should be NaN and is assumed to be used for time gaps in clouds, above the aircraft, and when side lobes are removed in curves"

            data_flag.values[
                np.isnan(radar_raw.dBZ) & (data_flag == Radar_flag.good)
            ] = Radar_flag.no_measurement

            gap_mask = find_sampling_gaps_eurec4a(data_flag)

        assert (
            Radar_flag.clear in data_flag.values
        ), "Could not find any clear sky. That is suspicious. Maybe the identification is wrong?"
        ...
        radar_mask_obseverd = calc_radar_mask(
            data_flag_observed, clutter_threshold=clutter_threshold
        )
        ...
        data_flag = data_flag_observed.interp(
            time=dBZ.time, method="nearest", kwargs=dict(fill_value=Radar_flag.no_measurement)
        )
        data_flag = data_flag.astype(int)
        assert set(Radar_flag).issuperset(
            np.unique(data_flag)
        ), "data_flag contains some non-Radar_flag values."

        radar_mask = radar_mask_obseverd.interp(
            time=dBZ.time, method="nearest", kwargs=dict(fill_value=Cloud_flag.unknown)
        )
        radar_mask = radar_mask.astype(int)
        assert set(Cloud_flag).issuperset(
            np.unique(radar_mask)
        ), "radar_mask contains some non-Cloud_flag values."
    ...
    dtype_flag = (
        np.int16
    )  # A int8 would be enough, but OPeNDAP can not handle negative int8, so we use int16 instead
    ...
    radar_mask_ds["cloud_top"] = (
        ["time"],
        radar_cloud_top_height.values,
        dict(
    ...
    flag_values = np.array(
        [Cloud_flag.clear, Cloud_flag.probably, Cloud_flag.certain], dtype=dtype_flag
    )
    radar_mask_ds["cloud_mask"] = (
        ["time"],
        radar_mask.values.astype(dtype_flag),
        dict(
    ...
    attrs["source"] = "From Cloud radar METEK MIRA35, based on " + " ".join(
        os.path.basename(s) for s in (radar_name, bahamas_name)
    )
    ...
    radar_mask_ds.to_netcdf(
        out_name.format(instrument="HAMP-Radar"), format="NETCDF4", encoding=encoding
    )

    radiometer_mask_ds = xarray.Dataset({"time": radar_mask_ds.time})
    radiometer_mask_ds["lat"] = radar_mask_ds.lat
    radiometer_mask_ds["lon"] = radar_mask_ds.lon

    rmv = radiometer_mask.values
    rmv[
        radiometer_mask_ds.lat > 20
    ] = Cloud_flag.unknown  # We don't use the LWP retrieval north of 20 deg N.
    flag_values = np.array(
        [Cloud_flag.clear, Cloud_flag.probably, Cloud_flag.certain], dtype=dtype_flag
    )
    radiometer_mask_ds["cloud_mask"] = (
        ["time"],
        rmv.astype(dtype_flag),
        dict(
            long_name="cloud flag",
            flag_values=flag_values,
            flag_meanings="no_cloud_detectable probably_cloudy most_likely_cloudy",
    ...
    attrs["source"] = "From Three RPG HALO MWR modules, based on " + " ".join(
        os.path.basename(s) for s in (retrieval_name, bahamas_name)
    )
    ...
    radiometer_mask_ds.to_netcdf(
        out_name.format(instrument="HAMP-MWR"), format="NETCDF4", encoding=encoding
    )
...
if __name__ == "__main__":
    ...
    path_retrieval_data = "/data/obs/campaigns/eurec4a/HALO/LWPIWV_CERA/v0.8/"
    path_unified = "/data/obs/campaigns/eurec4a/HALO/unified/v0.9/"
    path_output = "/net/blanc/awalbroe/Data/EUREC4A/HAMP_cloud_mask/"
    dates = [
        "20200119",
        "20200122",  # no radar
        "20200124",
    ...
    for date in dates:
        retrieval_name = glob.glob(path_retrieval_data + 
            f"EUREC4A_HALO_HAMP_lwpiwv_l2_any_v0.8_{date}[0-9][0-9][0-9][0-9][0-9][0-9].nc"
        )[0]
        bahamas_name = path_unified + f"bahamas_{date}_v0.9.nc"
        radar_name = path_unified + f"radar_{date}_v0.9.nc"
        out_name = path_output + f"EUREC4A_HALO_{{instrument}}_cloud_mask_{date}_v0.9.1.nc"  # uses int16 for cloud mask
        make_HAMP_cloudmask(
            retrieval_name,
            bahamas_name,
            radar_name,
