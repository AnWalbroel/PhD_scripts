...
def NN_retrieval(predictor_training, predictand_training, predictor_test,
	...
	test_loss = model.evaluate(predictor_test.input_scaled, predictand_test.iwv, verbose=0)
...
test_purpose = "Investigate rng and node number influence on fit of Neural Network with optimally working batch sizes, epochs and feature range for exponential"	# specify the intention of this test
...
mwr_dict['input'] = np.concatenate((mwr_dict['TBs'], mwr_dict['DOY_1'], mwr_dict['DOY_2']),			###############mwr_dict['pres']###################
										axis=1)
...
for aux_info['epochs'] in epochs:
	...
	for aux_info['seed'] in [12]:

		# seed for rng; default: 10; tests: 3,6,9,10,12,15,31,47,50,65,97,99
		...
		"""
		### The quality control of ERA-I data has been performed successfully and doesn't need to run every time from now on.
		...
		predictor_training.input = np.concatenate((predictor_training.TB, 			#########reshape_predictor_training.pres############
													predictor_training.DOY_1,
													predictor_training.DOY_2), axis=1)
		predictor_test.input = np.concatenate((predictor_test.TB, 						#########reshape_predictor_test.pres############
		...
		# if aux_info['epochs'] == 25:
			# fr_ranges = [(-3.0,1.0)]					# default range (0.0,1.0)
		# elif aux_info['epochs'] == 6:
			# fr_ranges = [(-3.0,0.0)]
		fr_ranges = [(-3.0,1.0)]
		...
		for fr in fr_ranges:
			...
			# if aux_info['epochs'] == 10:
				# BS = [1]
			# elif aux_info['epochs'] == 8:
				# BS = [8]
			...
			for aux_info['batch_size'] in BS:
				...
				# # # # # # # # # # epochs = [4]
				# epochs = [1,2,5,8,10,15,20,25]
				# epochs = [1,2,3,4,5,6,8,10]
...
RETRIEVAL_STAT_DS.attrs['input_vector'] = ("(TB_183.31+/-0.6, TB_183.31+/-1.5, TB_183.31+/-2.5, TB_183.31+/-3.5, TB_183.31+/-5.0, " +
											"TB_183.31+/-7.5, TB_243.00, TB_340.00, cos(DayOfYear), sin(DayOfYear))")						########pres_sfc#############
...
nc_output_name = "Retrieval_stat_test_" + "CONTROL" + "_"
if aux_info['considered_period'] == "leg%i"%(aux_info['mosaic_leg']):
