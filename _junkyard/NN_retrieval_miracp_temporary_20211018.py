...
def scatterplot_comparison(prediction_obs, mwr_dict, sonde_dict, aux_info, save_figures=True):
	...
	ax0.set_title("Retrieved (pred) vs. radiosonde (obs) IWV", fontsize=fs, pad=15.0)
...
def NN_retrieval(predictor_training, predictand_training, predictor_test,
	...
	# Create xarray to save predicted values:
	nc_output_name = f"NN_prediction_output_IWV_mosaic_{aux_info['considered_period']}_"
	NN_output_DS = xr.Dataset({'iwv':			(['time'], prediction_obs[:,0],
												{'description': "Predicted IWV from Neural Network model",
												'units': "mm"})},
								coords=			{'time': (['time'], mwr_dict['time'],
												{'description': "Time in seconds since 1970-01-01 00:00:00 UTC"})})

	NN_output_DS.attrs['batch_size'] = aux_info['batch_size']
	NN_output_DS.attrs['epochs'] = aux_info['epochs']
	NN_output_DS.attrs['activation'] = aux_info['activation']
	NN_output_DS.attrs['seed'] = aux_info['seed']
	NN_output_DS.attrs['feature_range'] = aux_info['feature_range']
	NN_output_DS.attrs['training_data'] = "Subset of ERA-Interim 2001-2017, 8 virtual stations north of 84.5 deg N"
	NN_output_DS.attrs['test_data'] = "Subset of ERA-Interim 2001-2017, 8 virtual stations north of 84.5 deg N"
	NN_output_DS.attrs['input_vector'] = ("(TB_183.31+/-0.6, TB_183.31+/-1.5, TB_183.31+/-2.5, TB_183.31+/-3.5, TB_183.31+/-5.0, " +
											"TB_183.31+/-7.5, TB_243.00, TB_340.00")
	NN_output_DS.attrs['author'] = "Andreas Walbroel, a.walbroel@uni-koeln.de"
	datetime_utc = dt.datetime.utcnow()
	NN_output_DS.attrs['datetime_of_creation'] = datetime_utc.strftime("%Y-%m-%d %H:%M:%S")

	if 'pres_sfc' in aux_info['predictors']:
		NN_output_DS.attrs['input_vector'] = NN_output_DS.input_vector + ", pres_sfc"

		nc_output_name = nc_output_name + "pres_sfc_"

	if ("DOY_1" in aux_info['predictors']) and ("DOY_2" in aux_info['predictors']):
		NN_output_DS.attrs['input_vector'] = NN_output_DS.input_vector + ", cos(DayOfYear), sin(DayOfYear)"
	NN_output_DS.attrs['input_vector'] = NN_output_DS.input_vector + ")"

	NN_output_DS.to_netcdf("/net/blanc/awalbroe/Data/NN_test/tests_overview/" + nc_output_name + 
							datetime_utc.strftime("%Y%m%d_%H%M") + ".nc", mode='w', format="NETCDF4")
	NN_output_DS.close()
...
aux_info['considered_period'] = "mosaic"	# specify which period shall be plotted or computed:
...
aux_info['batch_size'] = 8
aux_info['epochs'] = 25
aux_info['kernel_init'] = 'glorot_uniform'
