...
def run_HALO_raw_dropsonde_to_TB(
	...
	path_sfc_props : str, optional
		Surface properties from MIRS product
		"
		...
		"
		for sea ice concentration and emissivities. Default: None
	"""
	...
	if 'PAMTRA_DATADIR' not in os.environ:
		os.environ['PAMTRA_DATADIR'] = "/net/blanc/awalbroe/Data/pamtra_add_data/" # actual path is not required, but the variable has to be defined.
	import pyPamtra
	...
	# Import emissivity and sea ice concentration data from MIRS; later locate closest pixel to dropsonde, 
	# sorting the dataset by spatial distance to dropsonde. 
	if path_sfc_props:
		file_sfc_props = sorted(glob.glob(path_sfc_props + "MIRS_surf_props*.nc"))
		if len(file_sfc_props) == 1:
			DS_sfc_props = xr.open_dataset(file_sfc_props[0], decode_times=False)
			DS_sfc_props['time_npdt'] = DS_sfc_props.time.astype("datetime64[s]")

			# filter a bit where time are fill values:
			idx_time_okay = np.where(DS_sfc_props.time_npdt.values >= np.datetime64("2022-01-01"))[0]
			if len(idx_time_okay > 0):
				DS_sfc_props = DS_sfc_props.isel(Scanline=idx_time_okay)
		else:
			raise ValueError("Number of files of surface properties should be one.")
	...
	for filename_in in HALO_sondes_NC:
		...
		# HAMP FREQUENCIES:
		frq = [22.2400,23.0400,23.8400,25.4400,26.2400,27.8400,31.4000,50.3000,51.7600,
				52.8000,53.7500,54.9400,56.6600,58.0000,90.0000,110.250,114.550,116.450,
				117.350,120.150,121.050,122.950,127.250,175.810,178.310,179.810,180.810,
				181.810,182.710,183.910,184.810,185.810,186.810,188.310,190.810]
		...
		if isinstance(obs_height, str) and obs_height == 'BAHAMAS':
			...
			obs_height_value = np.unique(drop_alt)

		print(dropsonde_date + "\n")

		# surface type & reflectivity: Generally set to ocean, but identify
		# via land mask and sea ice maps if the sonde was released over those:
		if not path_sfc_props:
			...
			distance_threshold = 30		# in km

			# Further reduce size of MIRS data set:
			idx_time_okay = np.where((DS_sfc_props.time.values > pamData['timestamp'][0,0] - 43200) &
										(DS_sfc_props.time.values < pamData['timestamp'][0,0] + 43200))[0]
			if len(idx_time_okay) > 0:
				DS_sfc_reduced = DS_sfc_props.isel(Scanline=idx_time_okay)
			else:
				DS_sfc_reduced = deepcopy(DS_sfc_props)

			# resample to every 10th value to reduce computation time without missing the dropsonde location 
			# by much more
			idx_resampled = np.arange(0, len(DS_sfc_reduced.Scanline.values), 10)
			DS_sfc_reduced = DS_sfc_reduced.isel(Scanline=idx_resampled)

			# Sort surface property dataset by spatial distance to radiosonde, then find closest temporal overlap:
			geod_distance = np.ones((DS_sfc_reduced.Latitude.values.shape))*(-999)
			for scan in DS_sfc_reduced.Scanline.values:
				for fov in DS_sfc_reduced.Field_of_view.values:
					geod_distance[scan, fov] = (distance.distance((pamData['lat'][0], pamData['lon'][0]), 
												(DS_sfc_reduced.Latitude.values[scan,fov], DS_sfc_reduced.Longitude.values[scan,fov])).km)
			DS_sfc_reduced['distance_dropsonde'] = xr.DataArray(geod_distance, dims=["Scanline", "Field_of_view"], 
																attrs={'units': "km", 'comment': "Geodesic, WGS-84"})

			# identify indices closer than threshold:
			idx_distance = np.where(DS_sfc_reduced.distance_dropsonde.values <= distance_threshold)
			DS_sfc_reduced = DS_sfc_reduced.isel(Scanline=idx_distance[0], Field_of_view=idx_distance[1])
			if len(idx_distance[0]) > 0:	# then distance < threshold was found
				# then identify where temporal distance is minimal and then choose again the lowest distance:
				idx_time = np.argmin(np.abs(DS_sfc_reduced.time.values - pamData['timestamp'][0,0]))
				idx_min_distance = np.argmin(DS_sfc_reduced.distance_dropsonde.values[idx_time,:])
				DS_sfc_reduced = DS_sfc_reduced.isel(Scanline=idx_time, Field_of_view=idx_min_distance)

				# # # # # # # # # # # # emissivity_hamp_freq = np.interp(frq, DS_sfc_reduced.Freq.values, DS_sfc_reduced.Emis.values)								###################
				# # # # # # # # # # # # pam.nmlSet['emissivity'] = np.ones((1,1,2,len(frq),16))
				# # # # # # # # # # # # pam.nmlSet['emissivity'][:,:,0,:,-1] = emissivity_hamp_freq
				# # # # # # # # # # # # pam.nmlSet['emissivity'][:,:,1,:,-1] = emissivity_hamp_freq

				if float(DS_sfc_reduced.Sfc_type.values) in [0,1,3]:
			...
			else:					# MIRS data too far off dropsonde position, use "normal" PAMTRA data again
				pamData['sfc_type'] = np.zeros(shape2d)			# 0: ocean, 1: land
				pamData['sfc_refl'] = np.chararray(shape2d)
				pamData['sfc_refl'][:] = 'F'
				pamData['sfc_refl'][pamData['sfc_type'] == 1] = 'S'
		...
		distance_lat_squared = (np.pi*6371000*dlat/180)**2
		...
		# save output:
		filename_out = pam_out_path + os.path.basename(filename_in)[:-3] + "_pamtra.nc"
		pam.writeResultsToNetCDF(filename_out, xarrayCompatibleOutput=True, ncCompression=True)
		# pam.r['emissivity'].shape is x,y,polarization,freqs,angles, while here, LAST ANGLE == nadir, first angle = close to horizontal
